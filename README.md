...
- Giải thích ETL:
  `Extract -> Transform + ML -> Load`
- Show log trong Airflow UI
- Show query:
...
```sql
SELECT title, total_sold_count, ml_popularity_score, ml_predicted_popular
FROM shopee_clean
ORDER BY ml_popularity_score DESC
LIMIT 10;
```

## 10. Checklist debug Airflow ngắn gọn

Khi task fail, kiểm tra theo thứ tự này:

1. Mở `Task Instance Log` trong Airflow UI và đọc dòng lỗi cuối cùng trước.
2. Xác nhận container còn chạy:
   `docker compose ps`
3. Nếu lỗi DB connection:
   - Từ container Airflow, luôn dùng `postgres:5432`
   - Từ máy host, dùng port publish như `localhost:5434`
4. Kiểm tra env trong container:
   `docker compose exec airflow-webserver /bin/bash -lc 'env | grep WAREHOUSE_DB'`
5. Kiểm tra DAG import có lỗi không:
   `docker compose exec airflow-webserver airflow dags list-import-errors`
6. Kiểm tra DAG run/task state:
   `docker compose exec airflow-webserver airflow dags list-runs -d etl_shopee_pipeline -o json`
7. Nếu vừa sửa `docker-compose.yml`, recreate container để env mới có hiệu lực:
   `docker compose up -d --force-recreate airflow-webserver airflow-scheduler`
8. Nếu nghi dữ liệu đầu vào có vấn đề, kiểm tra file trong container:
   `docker compose exec airflow-webserver ls /opt/airflow/data`
9. Nếu chỉ một run lỗi do cấu hình cũ, xóa run fail trong metadata rồi trigger lại.
10. Nếu muốn demo lại từ batch đầu:
   - chạy:
     `docker compose exec airflow-webserver python /opt/airflow/scripts/reset_demo.py`
   - trigger lại DAG

Script reset sẽ:

- xóa file state `data/staging/shopee_demo_state.json`
- xóa artifact staging cũ
- drop bảng `shopee_raw`, `shopee_clean`, `shopee_clean_stage`
- giúp lần trigger tiếp theo bắt đầu lại từ batch đầu tiên

Tuỳ chọn:

- chỉ reset state/file, không đụng database:
  `docker compose exec airflow-webserver python /opt/airflow/scripts/reset_demo.py --skip-db`
- giữ lại artifact ML, chỉ reset state và bảng:
  `docker compose exec airflow-webserver python /opt/airflow/scripts/reset_demo.py --keep-artifacts`
